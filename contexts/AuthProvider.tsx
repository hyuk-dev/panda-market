...
export function AuthProvider({ children } : AuthProviderProps){
  ...
  const {data: userData, isLoading, isError} = useQuery<UserCard>({
    queryKey: ["userData"],
    queryFn: getUserData,
    enabled: !!accessToken,
    initialData: () => {
  ...
  async function refreshAccessToken() {
    try {
      const res = await api.post("https://panda-market-api.vercel.app/auth/refresh-token",{
        refreshToken,
      })
  ...
  return (
    <AuthContext.Provider value={{userData : userData ?? null, isLoading,  signout, refreshToken, accessToken, setAccessToken, setRefreshToken}}>
      {children}
